...
export interface AuthContextValue {
  ...
  isDemo: boolean;
  error: string | null;
  login: (userData: AuthUser | SessionData | Record<string, unknown>) => void;
  logout: () => Promise<void>;
  demoLogin: (role: "manager" | "hse_officer" | "employee" | string, employee_id?: string) => Promise<AuthUser | SessionData>;
  standardLogin?: (username: string) => Promise<AuthUser | SessionData>;
 ...
 */
export function isDemo(user?: unknown): boolean {
  if (!user || typeof user !== "object") return false;
  const u = user as Record<string, unknown>;
  return Boolean(
    u.is_demo === true ||
    u.isDemo === true ||
    (typeof u.id === "string" && u.id.startsWith("u-")) ||
    u.companyId === "c-apex-01" ||
    u.company_id === "c-apex-01"
  );
}
 ...
 */
export function login(user: unknown): void {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginState = (userData: AuthUser | SessionData | Record<string, unknown>) => {
    setUser(userData as AuthUser | SessionData);
  ...
  const refreshSession = async () => {
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : "Session error";
      if (process.env.NODE_ENV !== "test") {
        console.warn("Backend session check unavailable, defaulting to unauthenticated:", errMsg);
      }
      setError(errMsg);
      setUser(null);
    } finally {
  ...
  const demoLogin = async (role: "manager" | "hse_officer" | "employee" | string, employee_id?: string) => {
    ...
    setError(null);
    try {
      const data = await authApi.demoLogin(role as "manager" | "hse_officer" | "employee", employee_id);
      loginState(data);
      return data;
    } catch {
